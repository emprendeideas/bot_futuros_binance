...
if __name__ == "__main__":
    ...
    def run_bot():
        global ultima_senal_historica, klines, primera_senal_valida

        enviar_telegram("🤖 BOT CON FILTRO INTELIGENTE ACTIVADO")

        print("🚀 Inicio sin carga histórica", flush=True)

        # 🔥 VALIDACIÓN REAL (NO BLOQUEA BOT)
        if len(klines) < 50:
            print("⚠️ Histórico insuficiente. Bot iniciará SOLO con live data", flush=True)
            ultima_senal_historica = None
            primera_senal_valida = False
        else:
            print("✅ Histórico suficiente en memoria", flush=True)

        # 🔥 reconstrucción real tipo Pine
        if len(klines) >= 300:
            ultima_senal_historica = reconstruir_estado_pine()
        else:
            print("⚠️ No hay suficientes velas para reconstrucción, usando señal live", flush=True)
