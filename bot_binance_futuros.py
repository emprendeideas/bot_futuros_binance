...
def cargar_historico():
    ...
    # ===== BINANCE =====
    url_binance = f"https://fapi.binance.com/fapi/v1/klines?symbol={SYMBOL.upper()}&interval={INTERVAL}&limit=300"

    # ===== BYBIT =====
    url_bybit = f"https://api.bybit.com/v5/market/kline?category=linear&symbol={SYMBOL.upper()}&interval=1&limit=300"
    ...
    # =========================
    try:
        print("🔄 Intentando Binance...", flush=True)

        r = requests.get(url_binance, timeout=10, headers=headers)
        ...
        if r.status_code == 200:
            ...
            print("✅ HISTÓRICO BINANCE OK", flush=True)
            return

        else:
            print(f"⚠️ Binance fallo: {r.status_code}", flush=True)
    ...
    # =========================
    try:
        print("🔄 Intentando Bybit...", flush=True)
        ...
        if data["retCode"] != 0:
            print(f"❌ Bybit error: {data}", flush=True)
            return
        ...
        klines = [{
            ...
            "close": float(k[4]),
            "time": int(k[0])
        } for k in reversed(lista)]  # 👈 IMPORTANTE

        last_candle_time = klines[-1]["time"]

        print("✅ HISTÓRICO BYBIT OK (FALLBACK)", flush=True)
        return

    except Exception as e:
        print(f"❌ Bybit error: {e}", flush=True)

    # =========================
    # ❌ FALLÓ TODO
    # =========================
    print("❌ No se pudo cargar histórico de ninguna fuente", flush=True)
...
def on_message(ws, message):
    ...
    print("📩 Nueva vela cerrada recibida", flush=True)  # 🔥 DEBUG: vela cerrada (debe salir cada 1 min)
...
def iniciar_websocket():
    while True:
        try:
            print("🔌 Conectando WebSocket...", flush=True)

            ws = websocket.WebSocketApp(
                f"wss://fstream.binance.com/stream?streams={SYMBOL}@kline_{INTERVAL}",
                on_message=on_message,
                on_error=lambda ws, error: print(f"❌ WS Error: {error}", flush=True),
                on_close=lambda ws, a, b: print("⚠️ WebSocket cerrado", flush=True),
                on_open=lambda ws: print("✅ WebSocket conectado", flush=True)
            )

            ws.run_forever(ping_interval=20, ping_timeout=10)

        except Exception as e:
            print(f"❌ EXCEPCIÓN WS: {e}", flush=True)

        print("🔄 Reintentando conexión en 5s...", flush=True)
        time.sleep(5)
