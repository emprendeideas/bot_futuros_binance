...
def cargar_historico():
    ...
    try:
        print("🔄 Cargando histórico desde Binance (endpoint único)...", flush=True)

        r = requests.get(url, params=params, headers=headers, timeout=10)

        if r.status_code != 200:
            print(f"❌ Error histórico Binance → HTTP {r.status_code}", flush=True)
            klines = []
            last_candle_time = None
            return
        ...
        if not isinstance(data, list) or len(data) == 0:
            print("❌ Respuesta inválida de Binance", flush=True)
            klines = []
            last_candle_time = None
            return
        ...
        print(f"✅ HISTÓRICO CARGADO OK ({len(klines)} velas)", flush=True)

    except Exception as e:
        print(f"❌ Error histórico: {e}", flush=True)
        klines = []
        last_candle_time = None
...
if __name__ == "__main__":
    ...
    def run_bot():
        ...
        if len(klines) < 50:
            print("⚠️ Histórico insuficiente. Iniciando construcción desde live data...", flush=True)
