...
def filtro_rango(señal, precio):
    global last_entry_price, last_entry_type

    nueva = 1 if señal == "BUY" else -1

    # Primera señal pasa siempre
    if last_entry_type == 0:
        last_entry_price = precio
        last_entry_type = nueva
        return True

    # Debe ser opuesta
    if nueva == last_entry_type:
        return False

    # PnL respecto a memoria (igual que Pine)
    if last_entry_type == 1:  # BUY en memoria
        distancia = (precio - last_entry_price) / last_entry_price * 100
    else:  # SELL en memoria
        distancia = (last_entry_price - precio) / last_entry_price * 100
...
def cargar_historico():
    ...
    for intento in range(5):  # reintentos
        try:
            ...
            # ⚠️ Validar que sea lista de klines
            if not isinstance(data, list):
                print(f"[ERROR BINANCE] Respuesta inválida: {data}")
                time.sleep(2)
                continue

            # ⚠️ Validar estructura interna
            if len(data) == 0 or not isinstance(data[0], list):
                print(f"[ERROR FORMATO] {data}")
                time.sleep(2)
                continue
    ...
    # ❌ Si falla todo
    raise Exception("No se pudo cargar histórico después de varios intentos")
...
def on_message(ws, message):
    ...
    if not filtro_rango(señal, ultimo_precio):
        return
