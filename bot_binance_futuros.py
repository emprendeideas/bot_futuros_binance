...
def polling_loop():
    ...
    print("🟢 INICIANDO POLLING SPOT...", flush=True)

    while True:
        try:
            url = f"https://api.binance.me/api/v3/klines?symbol={SYMBOL.upper()}&interval={INTERVAL}&limit=2"
            ...
            if not isinstance(data, list) or len(data) == 0:
                print(f"⚠️ Respuesta inválida: {data}", flush=True)
                time.sleep(2)
                continue
            ...
            print("📩 Nueva vela detectada (SPOT)", flush=True)
            ...
            if not bot_activo or detener_bot_total:
                continue
