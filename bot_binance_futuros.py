...
def verificar_stop_loss():
    ...
    if pnl <= SL_PORCENTAJE:
        # aplicar pérdida fija igual que Pine
...
def cargar_historico():
    ...
    # =========================
    # 🔥 BINANCE MULTI-ENDPOINT
    # =========================
    for base_url in endpoints_binance:
        try:
            url = f"{base_url}/fapi/v1/klines?symbol={SYMBOL.upper()}&interval={INTERVAL}&limit=1000"

            print(f"🔄 Intentando Binance endpoint: {base_url}", flush=True)
            ...
            if r.status_code == 200:
                ...
                print(
                    f"✅ HISTÓRICO BINANCE OK desde {base_url}",
                    flush=True
                )
                return
                ...
                print(
                    f"⚠️ Endpoint falló {base_url} → {r.status_code}",
                    flush=True
                )

        except Exception as e:
            print(
                f"❌ Error endpoint {base_url}: {e}",
                flush=True
            )

    # =========================
    # ❌ SI BINANCE FALLA → NO OPERAR
    # =========================
    print("❌ No se pudo cargar histórico desde Binance Futures", flush=True)
...
def on_message(ws, message):
    try:
        ...
        print(f"📦 RAW: {message[:120]}", flush=True)
        print("📩 MENSAJE CRUDO:", message[:200], flush=True)
        ...
        # 🔥 IMPORTANTE (nuevo formato stream)
        if "data" in data and "k" in data["data"]:
            k = data["data"]["k"]
        else:
            print("⚠️ ignorado:", data, flush=True)
            return

        except Exception as e:
            print(f"❌ Error parseando mensaje WS: {e}", flush=True)
            return

        print(f"[DEBUG] x={k['x']}", flush=True)

        if not k["x"]:
            return

        print("📩 Nueva vela cerrada recibida", flush=True)
        print(f"💰 Precio cierre: {k['c']}", flush=True)

        # Vela cerrada
        ultimo_precio = float(k["c"])
        ...
        print(f"📊 Total velas: {len(klines)}", flush=True)

        if not bot_activo:
            return

        señal = calcular_senal_final()
        print(f"🧠 Señal calculada: {señal}", flush=True)
        ...
        print(f"❌ ERROR en on_message: {e}", flush=True)

def polling_loop():
    global last_candle_time, ultimo_precio, primera_senal_valida
    ...
    while True:
        try:
            ...
            # 🔁 MULTI-ENDPOINT (anti caídas Binance)
            for base_url in endpoints:
                try:
                    url = f"{base_url}/fapi/v1/klines?symbol={SYMBOL.upper()}&interval={INTERVAL}&limit=2"

                    r = requests.get(url, timeout=5)

                    if r.status_code != 200:
                        print(f"⚠️ HTTP {r.status_code} en {base_url}", flush=True)
                        continue
                    ...
                    if not isinstance(temp_data, list) or len(temp_data) == 0:
                        print(f"⚠️ Respuesta inválida en {base_url}: {temp_data}", flush=True)
                        continue

                    data = temp_data
                    break  # ✔ endpoint válido encontrado

                except Exception as e:
                    print(f"❌ Error endpoint {base_url}: {e}", flush=True)

            if data is None:
                print("❌ Ningún endpoint respondió correctamente", flush=True)
                time.sleep(2)
                continue
            ...
            print("📩 Nueva vela detectada (REST)", flush=True)

            last_candle_time = candle_time
            ultimo_precio = float(k[4])
            ...
            # 🔒 evitar señal histórica repetida
            if not primera_senal_valida:
                if señal != ultima_senal_historica:
                    primera_senal_valida = True
                else:
...
if __name__ == "__main__":
    ...
    def run_bot():
        ...
        # 🔁 CARGA CONTROLADA (SIN SPAM)
        cargar_historico()

        if len(klines) < 200:
            print("❌ No se pudo cargar histórico suficiente. Reiniciar deploy.", flush=True)
            return

        print("✅ Histórico suficiente cargado", flush=True)

        # 🔥 reconstrucción real tipo Pine
        ...
        # 🔥 POLLING COMO PROCESO PRINCIPAL (CLAVE)
        polling_loop()
