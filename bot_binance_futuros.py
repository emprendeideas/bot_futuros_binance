...
def cargar_historico():
    ...
    for intento in range(5):  # máximo 5 intentos
        try:
            print(f"🔄 Intentando cargar histórico... (intento {intento+1})", flush=True)

            response = requests.get(url, timeout=10, headers=headers)

            # 🚫 BLOQUEOS BINANCE
            if response.status_code in [418, 403]:
                print("🚫 Binance bloqueando (418/403). Esperando 15 minutos...", flush=True)
                time.sleep(900)
                return  # 🔥 salir y dejar que el loop externo controle
    ...
    # ❌ SI FALLA TODO
    print("❌ No se pudo cargar histórico después de 5 intentos", flush=True)

    # 🔥 LIMPIAR ESTADO (importante)
    klines = []
    last_candle_time = None
...
if __name__ == "__main__":
    ...
    def run_bot():
        ...
        # 🔁 CARGA DE HISTÓRICO CON BACKOFF INTELIGENTE
        ...
        while True:
            ...
            print(f"⏳ Falló histórico. Reintentando en {delay}s...", flush=True)
            time.sleep(delay)

            # 🔥 BACKOFF EXPONENCIAL (anti-ban)
            delay = min(delay * 2, 600)  # máximo 10 minutos

        # 🔥 reconstrucción inicial tipo Pine
        ultima_senal_historica = reconstruir_estado_pine()

        # 🔥 sincroniza con estado actual
        ...
        # 🔥 WEBSOCKET (ya maneja reconexión solo)
        iniciar_websocket()
