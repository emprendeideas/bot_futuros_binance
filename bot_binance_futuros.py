...
def iniciar_websocket():
    ...
    while True:
        try:
            print("🔌 Conectando WebSocket Binance...", flush=True)

            socket_url = f"wss://fstream.binance.com/stream?streams={SYMBOL}@kline_{INTERVAL}"

            def on_open(ws):
                print("✅ WebSocket conectado", flush=True)

            def on_close(ws, close_status_code, close_msg):
                print(f"⚠️ WS cerrado | code={close_status_code} msg={close_msg}", flush=True)
            ...
            while True:
                ...
                print("⚠️ run_forever terminó → reconectando...", flush=True)
                time.sleep(2)

                # 🔥 WATCHDOG REAL
                if time.time() - last_ws_message_time > 60:
                    print("⚠️ WS congelado → forzando reconexión...", flush=True)
                    break  # rompe loop interno y reconecta limpio

        except Exception as e:
            print(f"❌ EXCEPCIÓN GLOBAL WS: {e}", flush=True)
...
if __name__ == "__main__":
    ...
    def run_bot():
        ...
        # 🔥 SOLO ARRANCA SI TODO ESTÁ BIEN
        iniciar_websocket()
